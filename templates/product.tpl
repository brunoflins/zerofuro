                                ...
                                <div class="price product-price-container m-top-half text-left-xs">
                                    ...
                                    <span class="price product-price js-price-display" id="price_display" {% if not product.display_price %}style="display:none;"{% endif %} data-product-price="{{ product.price }}">
                                        {% if product.display_price %}
                                            {{ (product.price * 2) | money }}
                                        {% endif %}
                                    </span>
                                    <input type="hidden"  id="original_price" value="{{product.price}}"/>
                                </div>
                        ...
                        {% if product.available and product.display_price %}
                            <div class="js-product-quantity-container row-fluid quantity-container {% if settings.shipping_calculator_product_page and not product.free_shipping %}border-bottom-none-xs m-bottom-none{% endif %}">
                                <div class="quantity span3" data-component="product.adding-amount">
                                    ...
                                    <select onchange="selecionarQuantidade()" id="quantity" style="width: 50px;" name="quantity{{ item.id }}" value="{{ item.quantity }}" pattern="[0-9]" data-component="adding-amount.value">
                                        <option>2</option>
                                        <option>4</option>
                                    </select>
                                    ...
                                    <script>
                                        function selecionarQuantidade(){
                                            var precoOriginal = document.getElementById('original_price').value;
                                            var quantidade = document.getElementById('quantity').value;
                                            var total = precoOriginal * quantidade;

                                            let numStr = total.toString();

                                            // Ensure the string has at least two digits
                                            if (numStr.length < 2) {
                                            ...
                                            document.getElementById('price_display').innerHTML = 'R$'+ result;
                                        }
                                    </script>
                            ...
                            <div class="row-fluid">
                                <div class="product-buy-container span12">
                                    <div class="product-fixed-info pull-left">
                                        ...
                                        <span id="preco-label" class="price product-price product-price-fixed h2 js-price-display row-fluid" {% if not product.display_price %}style="display:none;"{% endif %}>
                                            {% if product.display_price %}
                                                {{ (product.price * 2) | money }}
                                            {% endif %}
                                        </span>
                                    ...
                                    <input type="submit" id="comprar" class="product-buy-btn btn btn-primary js-prod-submit-form js-addtocart span12 {{state}}" value="{{ texts[state] | translate }}" {% if state == 'nostock' %}disabled{% endif %} data-store="product-buy-button" data-component="product.add-to-cart"/>
